...
func longDescription() string {
	...
	longText += `EXAMPLE:
    List the versions of "opentofu"`

	longText = fmt.Sprintf("%s\n\n    > %s\n\n", longText,
		yellow("binary-version-switcher opentofu versions"))

	longText += `EXAMPLE:
    Activate a specific version of "opentofu"`

	longText = fmt.Sprintf("%s\n\n    > %s\n\n", longText,
		yellow("binary-version-switcher opentofu activate -v \"v1.7.0\""))
...
var MainCmd = &cobra.Command{
	Use:   "opentofu",
	Short: "Switch between versions of opentofu",
